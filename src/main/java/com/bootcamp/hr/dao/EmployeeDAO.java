...
public class EmployeeDAO {
    ...
    private static final String SELECT_EMPLOYEE_BY_EMPID_DEPTID_JOBID = "SELECT employee_id, first_name, last_name, email, salary, d.department_id, d.department_name, j.job_id, j.job_title FROM employees e INNER JOIN departments d ON d.department_id = e.department_id INNER JOIN jobs j ON j.job_id = e.job_id";
    ...
    private static final int DEPARTMENT_ID_COLUMN = 6;
    private static final int DEPARTMENT_NAME_COLUMN = 7;
    private static final int JOB_ID_COLUMN = 8;
    private static final int JOB_NAME_COLUMN = 9;
    ...
    public List<Employee> findEmployeeInfo () {
    	 List<Employee> employees = new ArrayList<>();
    	 try {
             statement = databaseConnectionManager.getConnection().prepareStatement(SELECT_EMPLOYEE_BY_EMPID_DEPTID_JOBID);
             resultSet = statement.executeQuery();
             while (resultSet.next()) {
                 Employee employee = toEmployeeInfo(resultSet);
                 employees.add(employee);
             }
         } catch (SQLException e) {
             this.logSQLException(e);
    ...
    private Employee toEmployeeInfo(ResultSet row) throws SQLException {
        Employee employee = new Employee();
        employee.setId(row.getInt(EMPLOYEE_ID_COLUMN));
        employee.setFirstName(row.getString(FIRST_NAME_COLUMN));
        employee.setLastName(row.getString(LAST_NAME_COLUMN));
        employee.setEmail(row.getString(EMAIL_COLUMN));
        employee.setSalary(row.getFloat(SALARY_COLUMN));
        employee.setDeptId(row.getInt(DEPARTMENT_ID_COLUMN));
        employee.setDeptName(row.getString(DEPARTMENT_NAME_COLUMN));
        employee.setJobId(row.getString(JOB_ID_COLUMN));
        employee.setJobName(row.getString(JOB_NAME_COLUMN));
        return employee;
    }
